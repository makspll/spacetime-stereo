...
if __name__ == "__main__":
    ...
    for epoch in range(training_epoch_start, epochs + 1):
        ...
        if args.local_rank == -1 or torch.distributed.get_rank() == 0:
            ...
            taken = end-start
            print(f"====> Epoch {epoch}: Time: {taken:.2f}s, Acc. train: {acc_t:.2f}, Acc. val: {acc_v:.2f}, Loss train: {loss_t:.2f}, Loss Val: {loss_v:.2f}  lr: {scheduler.get_last_lr()[-1]:.4f}, ETA: {((taken) * (epochs - epoch)) / 60 / 60:.2f}h")
