...
class MatchingCell(nn.Module):
    def __init__(self, c_in_prev_prev,c_in_prev,c_out,scale):
        ...
        self.pre_preprocess = ConvBR(c_in_prev_prev, c_out, kernel_size=1, stride=1, padding=0,dim=3)
        self.preprocess = ConvBR(c_in_prev, c_out, kernel_size=1, stride=1, padding=0,dim=3)
        self.scale = scale 
        self.c_out = c_out
