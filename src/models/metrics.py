...
def bad_n_error(n, disp, gt, area : AreaSource = AreaSource.BOTH, fg_mask = None, max_val=None,min_val=None):
    ...
    if max_val:
        gt_cp[gt_cp > max_val] = 0
    if min_val:
        gt_cp[gt_cp < min_val] = 0

    valid_gt = np.not_equal(gt_cp,0).astype(int)
